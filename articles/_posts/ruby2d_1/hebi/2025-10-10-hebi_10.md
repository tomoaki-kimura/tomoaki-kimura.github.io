...
chapter: 1
section: 10
section_title: アイテム取得後に胴体を伸ばす
description: アイテムとの当たり判定を作り、取得するとヘビの胴体が伸びるようにします。
---
...
# アイテムの当たり判定
def get_item(items, head, bodies, speed, logs)
  items.each do |item|
    if item.first.contains?(head.x, head.y)
      2.times do
        speed.times { |i| logs << [logs.last.first, logs.last.last] }
        bodies << Circle.new(color: "lime", radius: 8, x: logs.last.first, y: logs.last.last)
      end
      item.first.remove
      items.delete(item)
    end
...
(略)
    ...
    # アイテムの取得
    get_item(items, head, bodies, speed, logs)
...
- 現在１アイテムに付き胴体が２こ増えるが、この数を変更してみる。
