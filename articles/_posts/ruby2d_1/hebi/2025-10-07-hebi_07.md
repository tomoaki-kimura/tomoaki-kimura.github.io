---
layout: post
part: Ruby2D(1)
title: ヘビゲーム
categories: [ruby2d_1, hebi]
chapter: 1
section: 7
section_title: 追従する胴体を作る
description: 頭の座標の履歴を配列に記録し、それを使って胴体が追従して動くようにします。
---

## {{ page.section_title }}

### 追従用の座標群を作る

main.rb
```
require "ruby2d"

# ヘビの頭
head = Circle.new(radius: 10)
head.x = Window.width / 2
head.y = Window.height / 2
speed = 1
x_flag = true
y_flag = false
status = :stop

# ヘビの胴体
body_size = 5
logs = (body_size * speed + 1).times.map { |i| [head.x - speed * i, head.y + speed * i] }

# ゲームオーバー画面
message = [
(略)

(略)
    else
      head.y -= speed
    end
    # logの更新
    logs.unshift([head.x, head.y])
    logs.pop
    p logs # 追従用の座標群
    p [head.x, head.y] # 頭の座標
  end
end

show
```

### ファイルの実行

bash
```
$ ruby main.rb
```

### 結果

表示は変わらないが、コンソールに追従用の座標群と、頭の座標が表示される。

#### *ここで試したいこと

- ゲームオーバー時点で、頭の座標が座標群の最初のデーターと一致していることを確認する。

### 胴体を作る

main.rb
```
require "ruby2d"

# ヘビの頭
head = Circle.new(radius: 10)
head.x = Window.width / 2
head.y = Window.height / 2
speed = 1
x_flag = true
y_flag = false
status = :stop

# ヘビの胴体
body_size = 5
logs = (body_size * speed + 1).times.map { |i| [head.x - speed * i, head.y + speed * i] }
bodies = body_size.times.map { |i| Circle.new(radius: 8, x: logs[speed * i].first, y: logs[speed * i].last) }


# ゲームオーバー画面
message = [
(略)

(略)
    else
      head.y -= speed
    end
    # logの更新
    logs.unshift([head.x, head.y])
    logs.pop
    p logs # 追従用の座標群
    p [head.x, head.y] # 頭の座標
    p bodies.map { |o| [o.x, o.y] } # 胴体の座標群
  end
end

show
```

### ファイルの実行

bash
```
$ ruby main.rb
```

### 結果

胴体が作られ、頭についてくる。

#### *ここで試したいこと

- スピードを変更して動きを確認する。

### スピードの変更

main.rb
```
require "ruby2d"

# ヘビの頭
head = Circle.new(radius: 10)
head.x = Window.width / 2
head.y = Window.height / 2
speed = 3
x_flag = true
(略)
```

### ファイルの実行

bash
```
$ ruby main.rb
```

### 結果

スピードが適正化され、胴体が見やすくなる。

### ここまでのコード

main.rb
```
require "ruby2d"

# ヘビの頭
head = Circle.new(radius: 10)
head.x = Window.width / 2
head.y = Window.height / 2
speed = 3
x_flag = true
y_flag = false
status = :stop

# ヘビの胴体
body_size = 5
logs = (body_size * speed + 1).times.map { |i| [head.x - speed * i, head.y + speed * i] }
bodies = body_size.times.map { |i| Circle.new(radius: 8, x: logs[speed * i].first, y: logs[speed * i].last) }

# ゲームオーバー画面
message = [
  Text.new("GAME OVER", size: 30, x: 220, y: 150),
  Text.new("press key 'R' to restart", size: 20, x: 205, y: 300)
]
message.each { |text| text.remove }

def hit(head)
  # 上の当たり判定
  top = head.contains?(head.x, 0)
  # 下の当たり判定
  bottom = head.contains?(head.x, Window.height)
  # 右の当たり判定
  right = head.contains?(Window.width, head.y)
  # 左の当たり判定
  left = head.contains?(0, head.y)
  top || bottom || right || left
end

on :key_down do |event|
  # ゲームスタート
  if event.key == "right" || event.key == "left"
    status = :game_start if status == :stop
  end
  # リスタート
  if event.key == "r" && status == :game_over
    status = :stop
    head.x = Window.width / 2
    head.y = Window.height / 2
    x_flag = true
    y_flag = false
    logs = (body_size * speed + 1).times.map { |i| [head.x - speed * i, head.y + speed * i] }
    bodies.each { |body| body.remove }
    bodies = body_size.times.map { |i| Circle.new(radius: 8, x: logs[speed * i].first, y: logs[speed * i].last) }
    message.each { |text| text.remove }
  end
  # 右上に移動の時
  if x_flag && !y_flag
    case event.key
    when "right"
      y_flag = true
    when "left"
      x_flag = false
    end
  # 左上に移動の時
  elsif !x_flag && !y_flag
    case event.key
    when "right"
      x_flag = true
    when "left"
      y_flag = true
    end
  # 右下に移動の時
  elsif x_flag && y_flag
    case event.key
    when "right"
      x_flag = false
    when "left"
      y_flag = false
    end
  # 左下に移動の時
  elsif !x_flag && y_flag
    case event.key
    when "right"
      y_flag = false
    when "left"
      x_flag = true
    end
  end
end

update do
  case status
  when :game_start
    # 当たり判定
    if hit(head)
      status = :game_over
      message.each { |text| text.add }
    end
    # 移動方向の管理
    if x_flag
      head.x += speed
    else
      head.x -= speed
    end
    if y_flag
      head.y += speed
    else
      head.y -= speed
    end
    # logの更新
    logs.unshift([head.x, head.y])
    logs.pop
    # 胴体の更新
    bodies.each.with_index(1) do |body, i|
      body.x = logs[speed * i].first
      body.y = logs[speed * i].last
    end
    p logs # 追従用の座標群
    p [head.x, head.y] # 頭の座標
    p bodies.map { |o| [o.x, o.y] } # 胴体の座標群
  end
end

show
```
